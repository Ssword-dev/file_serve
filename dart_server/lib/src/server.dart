...
class DartServer {
  ...
  Future<void> _handleRequest(HttpRequest req, MimeGuesser mimeGuesser) async {
    ...
    final vfile = VirtualFile(owner: vfs, path: safePath);
    if (!await vfs.exists(vfile)) {
    ...
    try {
      ...
      final stream = await vfs.readFile(vfile);
  ...
  String _resolveSafePath(String requestPath) {
    ...
    return path.resolve(requestPath, rootDirectory.path);
  }
